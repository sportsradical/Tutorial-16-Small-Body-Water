...
bool SkyPlaneClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* cloudTextureFilename, char* perturbTextureFilename)
{
	...
	result = LoadTextures(device, deviceContext, cloudTextureFilename, perturbTextureFilename);
	if(!result)
	{
		return false;
	}
...
bool SkyPlaneClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* textureFilename1, char* textureFilename2)
{
	...
	result = m_CloudTexture->Initialize(device, deviceContext, textureFilename1);
	if(!result)
	{
		return false;
	}
	...
	result = m_PerturbTexture->Initialize(device, deviceContext, textureFilename2);
	if(!result)
	{
		return false;
	}
